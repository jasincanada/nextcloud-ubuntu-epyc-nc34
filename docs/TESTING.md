...
- [ ] Core upgrade path from NC32 (tested via backup/restore into this environment)
...
1. Take a PG dump from the NC32 stack:
   ```bash
   docker exec nextcloud-wsl2-enterprise-ai-infra-enabled-db-1 pg_dump -U nextcloud nextcloud > nc32_export.sql
   ```
2. Import into this stack's DB:
   ```bash
   docker exec -i nextcloud-ubuntu-epyc-nc34-db-1 psql -U nextcloud nextcloud < nc32_export.sql
   ```
